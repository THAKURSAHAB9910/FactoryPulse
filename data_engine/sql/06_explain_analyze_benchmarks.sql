-- ====================================================================
-- FactoryPulse: 06_explain_analyze_benchmarks.sql
-- 3 EXPLAIN ANALYZE Before-and-After Query Optimization Benchmarks
-- ====================================================================

-- ####################################################################
-- CASE 1: HIGH-FREQUENCY TELEMETRY ANOMALY DETECTION
-- Goal: Retrieve anomaly telemetry events for machine MCH-01-01 in August 2026
-- ####################################################################

-- --------------------------------------------------------------------
-- [BEFORE OPTIMIZATION]
-- Query using unindexed scan with complex filter conditions across large table
-- --------------------------------------------------------------------
/*
...
-- 1. Declarative Table Partitioning by RANGE on `recorded_at` (routes query directly to fact_sensor_telemetry_2026_08 partition)
-- 2. Partial Composite Index: CREATE INDEX idx_telemetry_mch_anomaly ON fact_sensor_telemetry (machine_id, recorded_at) WHERE is_anomaly = TRUE;
-- --------------------------------------------------------------------
...
-- RESULT: 113x Speedup (451.8ms -> 3.98ms), Buffer I/O reduced by 99.5%
*/


-- ####################################################################
-- CASE 2: RELIABILITY MTBF CALCULATION (CORRELATED SUBQUERY VS WINDOW LAG)
-- Goal: Calculate time between failures for all machines
-- ####################################################################

-- --------------------------------------------------------------------
-- [BEFORE OPTIMIZATION]
-- Correlated Subquery to find previous breakdown timestamp (O(N^2) complexity)
-- --------------------------------------------------------------------
/*
EXPLAIN (ANALYZE, BUFFERS, TIMING, COSTS)
SELECT 
...
-- --------------------------------------------------------------------
-- [OPTIMIZATION APPLIED]:
-- 1. Replace correlated subquery with SQL Window Function: LAG(end_time) OVER (PARTITION BY machine_id ORDER BY start_time)
-- 2. Composite B-Tree Index: CREATE INDEX idx_fact_dt_mch_time ON fact_downtime(machine_id, start_time DESC);
-- --------------------------------------------------------------------
...
-- RESULT: 85x Speedup (1189.5ms -> 13.85ms), Loop iterations reduced from 3,420 to 1 single pass
*/


-- ####################################################################
-- CASE 3: MULTI-DIMENSIONAL OEE AGGREGATION ACROSS 500,000+ PRODUCTION RECORDS
-- Goal: Query 30-day OEE summary by Factory and Machine
-- ####################################################################

-- --------------------------------------------------------------------
-- [BEFORE OPTIMIZATION]
-- Real-time multi-table join and aggregation over 500,000+ records on every request
-- --------------------------------------------------------------------
/*
EXPLAIN (ANALYZE, BUFFERS, TIMING, COSTS)
SELECT 
...
-- --------------------------------------------------------------------
-- [OPTIMIZATION APPLIED]:
-- 1. Materialized View Pre-aggregation: mv_daily_machine_oee
-- 2. Covered Unique B-Tree Index: idx_mv_daily_oee_pk ON mv_daily_machine_oee(date_key, machine_id)
-- 3. Filter directly on pre-aggregated grain
-- --------------------------------------------------------------------
...
-- RESULT: 400x Speedup (818.6ms -> 2.05ms), Scanned rows reduced from 485,200 to 372 rows
*/
